...
/**
 * This Class is the bridge between the hardware drivers and the program. 
 * The drivers execute certain methods when used, these are then called in this class.
 * For a driver to be able to use the methods in BicycleGarageManager it has to register the manager first.
 * This should be done right after initializing the object (driver).
 * The BicycleGarageManager also have to register the drivers so that it can access the public methods in those objects (drivers).
 * 
...
public class BicycleGarageManager {//TODO implement methods specified by course homepage: http://cs.lth.se/etsa01/projekt-2015/specifikation-av-mjukvaran/
	 ...
	 *
	 * @param printer BarcodePrinter that should be registered.
	 * @param entryLock ElectronicLock for the entry.
	 * @param exitLock ElectronicLock for the exit.
	 * @param terminal PinCodeTerminal for the entry door.
	 */
	...
	/** This method will be called when a user has used the bar code
	 * reader at the entry door. Bicycle ID should be a
	 * string of 5 characters, where every character
	 * can be '0', '1',... '9'.
	 *
	 * @param bicycleID String representing the ID for a bicycle
	 */
